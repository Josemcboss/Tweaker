    ...
    public static class ServiceOptimization
    {
        ...
        public static bool DisableDiagTrack()
        {
            return DisableService(
                ...
                displayName: "DiagTrack (Telemetría)",
                registryPath: @"SYSTEM\CurrentControlSet\Services\DiagTrack"
            );
        }
        ...
        private static bool DisableService(string serviceName, string displayName, string registryPath)
        {
            ...
            {
                // ???????????????????????????????????????????????????????????
                // PASO 1: DETENER EL SERVICIO (Efecto Inmediato)
                // ???????????????????????????????????????????????????????????
                ...
                // ???????????????????????????????????????????????????????????
                // PASO 2: DESHABILITAR VÍA REGISTRO (Permanente)
                // ???????????????????????????????????????????????????????????
