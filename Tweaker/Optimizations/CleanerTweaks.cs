...
namespace Tweaker.Optimizations
{
    ...
    /// Elimina archivos temporales y cach�
    /// </summary>
    public static class CleanerTweaks
    {
        ...
        ///    - Cach� de inicio de programas
        ///    - Puede causar problemas si est� corrupto
        /// 
        /// IMPACTO:
        ...
        /// ? Si un archivo est� en uso, lo salta (no crashea)
        ...
        private static (long bytesFreed, int filesDeleted) CleanDirectory(string path)
        {
            ...
            try
            {
                ...
                foreach (string file in files)
                {
                    try
                    {
                        // Obtener tama�o ANTES de borrar
                        ...
                        // Si llegamos aqu�, el archivo se borr� exitosamente
                        bytesFreed += fileSize;
                        filesDeleted++;
                    }
                ...
                // Intentar borrar directorios vac�os
                try
                {
                    string[] dirs = Directory.GetDirectories(path, "*", SearchOption.AllDirectories);
                    foreach (string dir in dirs.OrderByDescending(d => d.Length)) // M�s profundos primero
                    {
                        try
                        {
        ...
        /// <summary>
        /// FLUSH DNS CACHE
        /// Limpia cach� de resoluci�n de nombres
        /// 
        /// �Qu� es DNS Cache?
        /// ????????????????????????????????????????????????????????????????
        ...
        /// 
        /// �Por qu� limpiarlo?
        /// - Cach� corrupto causa "DNS_PROBE_FINISHED_NXDOMAIN"
        /// - Despu�s de cambiar DNS (Google, Cloudflare)
        /// - Despu�s de problemas de red
        /// 
        /// COMANDO:
        /// ipconfig /flushdns
        /// 
        /// IMPACTO:
        /// ? Resuelve errores de conexi�n
            ...
            try
            {
                ...
                using (Process proc = Process.Start(psi))
                {
                    ...
                    if (output?.Contains("Successfully flushed") == true)
                    {
                        ...
                        Debug.WriteLine("  Cach� de DNS limpiado");
                        return true;
                    }
                }
        ...
        /// <summary>
        /// AN�LISIS DE ESPACIO (sin borrar)
        /// Escanea cu�nto espacio se puede liberar
        /// </summary>
        public static (long mbCanFree, int filesCount) AnalyzeSpace()
        {
            ...
            try
            {
                ...
                Debug.WriteLine($"?? An�lisis: {totalFiles} archivos, {mb} MB se pueden liberar");
